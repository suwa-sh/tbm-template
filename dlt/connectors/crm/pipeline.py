"""
独自開発CRMシステムからデータを収集するパイプライン
"""

import dlt
from typing import Dict, List, Any, Iterator
from .mock_data import generate_mock_data

@dlt.source
def crm_source(api_key: str = None) -> Dict[str, Any]:
    """
    独自開発CRMシステムからデータを収集するソース
    
    Args:
        api_key: CRM APIキー（モックモードでは不要）
        
    Returns:
        Dict[str, Any]: リソースを含むソース定義
    """
    return {
        "users": users_resource(),
        "modules": modules_resource(),
        "infrastructure": infrastructure_resource(),
        "usage_metrics": usage_metrics_resource()
    }

@dlt.resource
def users_resource() -> Iterator[Dict[str, Any]]:
    """
    CRMシステムのユーザー情報を取得するリソース
    
    Yields:
        Dict[str, Any]: ユーザー情報
    """
    mock_data = generate_mock_data()
    for user in mock_data["users"]:
        yield user

@dlt.resource
def modules_resource() -> Iterator[Dict[str, Any]]:
    """
    CRMシステムのモジュール情報を取得するリソース
    
    Yields:
        Dict[str, Any]: モジュール情報
    """
    mock_data = generate_mock_data()
    for module in mock_data["modules"]:
        yield module

@dlt.resource
def infrastructure_resource() -> Dict[str, Any]:
    """
    CRMシステムのインフラ情報を取得するリソース
    
    Returns:
        Dict[str, Any]: インフラ情報
    """
    mock_data = generate_mock_data()
    return mock_data["infrastructure"]

@dlt.resource
def usage_metrics_resource() -> Dict[str, Any]:
    """
    CRMシステムの使用状況メトリクスを取得するリソース
    
    Returns:
        Dict[str, Any]: 使用状況メトリクス
    """
    mock_data = generate_mock_data()
    return mock_data["usage_metrics"]

if __name__ == "__main__":
    # パイプラインの実行
    pipeline = dlt.pipeline(
        pipeline_name="crm_data",
        destination="postgres",
        dataset_name="crm"
    )
    
    # データのロード
    load_info = pipeline.run(crm_source())
    print(f"CRMデータのロード結果: {load_info}")
